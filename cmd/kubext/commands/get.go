...
func NewGetCommand() *cobra.Command {
	...
	var command = &cobra.Command{
		Use:   "get WORKFLOW",
		Short: "display details about a managed",
...
func printManaged(wf *wfv1.Managed, outFmt string) {
	switch outFmt {
	case "name":
	...
	case "wide", "":
		printManagedHelper(wf, outFmt)
	default:
		log.Fatalf("Unknown output format: %s", outFmt)
	}
}

func printManagedHelper(wf *wfv1.Managed, outFmt string) {
	const fmtStr = "%-20s %v\n"
...
type nodeInfoInterface interface {
	getID() string
	getNodeStatus(wf *wfv1.Managed) wfv1.NodeStatus
	getStartTime(wf *wfv1.Managed) metav1.Time
}
...
func (n *nodeInfo) getNodeStatus(wf *wfv1.Managed) wfv1.NodeStatus {
	return wf.Status.Nodes[n.id]
}

func (n *nodeInfo) getStartTime(wf *wfv1.Managed) metav1.Time {
...
type renderNode interface {
	...
	renderNodes(w *tabwriter.Writer, wf *wfv1.Managed, depth int, nodePrefix string, childPrefix string, outFmt string)
	nodeInfoInterface
}
...
func insertSorted(wf *wfv1.Managed, sortedArray []renderNode, item renderNode) []renderNode {
...
func attachToParent(wf *wfv1.Managed, n renderNode,
...
func convertToRenderTrees(wf *wfv1.Managed) map[string]renderNode {
	...
	for id, status := range wf.Status.Nodes {
		if status.Type == "" {
			log.Fatal("Missing node type in status node. Cannot get manageds created with Kubext <= 2.0 using the default or wide output option.")
			return nil
		}
...
func renderChild(w *tabwriter.Writer, wf *wfv1.Managed, nInfo renderNode, depth int,
...
func printNode(w *tabwriter.Writer, wf *wfv1.Managed, node wfv1.NodeStatus, depth int, nodePrefix string, childPrefix string, outFmt string) {
...
// boundaryNode
func (nodeInfo *boundaryNode) renderNodes(w *tabwriter.Writer, wf *wfv1.Managed, depth int, nodePrefix string, childPrefix string, outFmt string) {
...
func (nodeInfo *nonBoundaryParentNode) renderNodes(w *tabwriter.Writer, wf *wfv1.Managed, depth int, nodePrefix string, childPrefix string, outFmt string) {
	filtered, childIndent := filterNode(nodeInfo.getNodeStatus(wf))
	if !filtered {
		printNode(w, wf, nodeInfo.getNodeStatus(wf), depth, nodePrefix, childPrefix, outFmt)
	}
...
// executionNode
func (nodeInfo *executionNode) renderNodes(w *tabwriter.Writer, wf *wfv1.Managed, depth int, nodePrefix string, childPrefix string, outFmt string) {
