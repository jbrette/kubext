...
func NewListCommand() *cobra.Command {
	...
	var command = &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			var wfClient v1alpha1.ManagedInterface
			if listArgs.allNamespaces {
				wfClient = InitManagedClient(apiv1.NamespaceAll)
			} else {
			...
			var manageds []wfv1.Managed
			if listArgs.since == "" {
				manageds = wfList.Items
			} else {
				manageds = make([]wfv1.Managed, 0)
				minTime := parseSince(listArgs.since)
				for _, wf := range wfList.Items {
					if wf.Status.FinishedAt.IsZero() || wf.ObjectMeta.CreationTimestamp.After(minTime) {
						manageds = append(manageds, wf)
					}
				}
			}
			sort.Sort(ByFinishedAt(manageds))

			switch listArgs.output {
			case "", "wide":
				printTable(manageds, &listArgs)
			case "name":
	...
	command.Flags().BoolVar(&listArgs.allNamespaces, "all-namespaces", false, "Show manageds from all namespaces")
	command.Flags().StringVar(&listArgs.status, "status", "", "Filter by status (comma separated)")
	command.Flags().BoolVar(&listArgs.completed, "completed", false, "Show only completed manageds")
	command.Flags().BoolVar(&listArgs.running, "running", false, "Show only running manageds")
	command.Flags().StringVarP(&listArgs.output, "output", "o", "", "Output format. One of: wide|name")
	command.Flags().StringVar(&listArgs.since, "since", "", "Show only manageds newer than a relative duration")
	return command
}
...
func printTable(wfList []wfv1.Managed, listArgs *listFlags) {
...
func countCompletedRunning(wf *wfv1.Managed) (int, int) {
	completed := 0
	running := 0
...
type ByFinishedAt []wfv1.Managed
...
// managedStatus returns a human readable inferred managed status based on managed phase and conditions
func worklowStatus(wf *wfv1.Managed) wfv1.NodePhase {
	switch wf.Status.Phase {
	case wfv1.NodeRunning:
		if common.IsManagedSuspended(wf) {
