...
var (
	...
	wfClient         v1alpha1.ManagedInterface
...
// InitManagedClient creates a new client for the Kubernetes Managed CRD.
func InitManagedClient(ns ...string) v1alpha1.ManagedInterface {
	if wfClient != nil {
		return wfClient
	}
	initKubeClient()
...
// splitYAMLFile is a helper to split a body into multiple managed objects
func splitYAMLFile(body []byte) ([]wfv1.Managed, error) {
	manifestsStrings := yamlSeparator.Split(string(body), -1)
	manifests := make([]wfv1.Managed, 0)
	for _, manifestStr := range manifestsStrings {
		...
		if wf.Kind != "" && wf.Kind != managed.Kind {
			// If we get here, it was a k8s manifest which was not of type 'Managed'
			// We ignore these since we only care about validating Managed manifests.
			continue
		}
		if err != nil {
...
//flag or the default kubext installation namespace (kube-system)
