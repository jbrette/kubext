...
func NewLogsCommand() *cobra.Command {
	...
	var command = &cobra.Command{
		...
		Short: "print the logs for a container in a managed",
		Run: func(cmd *cobra.Command, args []string) {
			...
			if managed {
				err = printer.PrintManagedLogs(args[0])
				errors.CheckError(err)
			} else {
	...
	command.Flags().BoolVarP(&managed, "managed", "w", false, "Specify that whole managed logs should be printed")
...
// PrintManagedLogs prints logs for all managed pods
func (p *logPrinter) PrintManagedLogs(managed string) error {
	wfClient := InitManagedClient()
	wf, err := wfClient.Get(managed, metav1.GetOptions{})
	if err != nil {
		return err
	}
	timeByPod := p.printRecentManagedLogs(wf)
	if p.follow && wf.Status.Phase == v1alpha1.NodeRunning {
		p.printLiveManagedLogs(wf, timeByPod)
	}
	return nil
}
...
// Prints logs for managed pod steps and return most recent log timestamp per pod name
func (p *logPrinter) printRecentManagedLogs(wf *v1alpha1.Managed) map[string]*time.Time {
...
// Prints live logs for managed pods, starting from time specified in timeByPod name.
func (p *logPrinter) printLiveManagedLogs(managed *v1alpha1.Managed, timeByPod map[string]*time.Time) {
	logs := make(chan logEntry)
	streamedPods := make(map[string]bool)

	processPods := func(wf *v1alpha1.Managed) {
	...
	processPods(managed)
	informer := p.setupManagedInformer(managed.Namespace, managed.Name, func(wf *v1alpha1.Managed, done bool) {
		if done {
			close(logs)
		} else {
			processPods(wf)
