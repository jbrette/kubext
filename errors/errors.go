...
func New(code string, message string) error {
	...
	return kubexterr{code, message, err.(stackTracer)}
}
...
func Cause(err error) error {
	if kubextErr, ok := err.(kubexterr); ok {
		return errors.Cause(kubextErr.stracer)
	}
	return errors.Cause(err)
}

func (e kubexterr) Error() string {
	return e.message
}

func (e kubexterr) Code() string {
	return e.code
}

func (e kubexterr) Message() string {
	return e.message
}

func (e kubexterr) StackTrace() errors.StackTrace {
	return e.stracer.StackTrace()
}

func (e kubexterr) JSON() []byte {
	type errBean struct {
...
func (e kubexterr) Format(s fmt.State, verb rune) {
	switch verb {
	case 'v':
		if s.Flag('+') {
