...
// Managed is the definition of a managed resource
// +genclient
// +genclient:noStatus
...
type Managed struct {
	...
	Status            ManagedStatus `json:"status"`
}

// ManagedList is list of Managed resources
// +k8s:deepcopy-gen:interfaces=k8s.io/apimachinery/pkg/runtime.Object
...
// ManagedSpec is the specification of a Managed.
type ManagedSpec struct {
	// Templates is a list of managed templates used in a managed
	Templates []Template `json:"templates"`

	// Entrypoint is a template reference to the starting point of the managed
	Entrypoint string `json:"entrypoint"`

	// Arguments contain the parameters and artifacts sent to the managed entrypoint
	// Parameters are referencable globally using the 'managed' variable prefix.
	// e.g. {{managed.parameters.myparam}}
	Arguments Arguments `json:"arguments,omitempty"`

	// ServiceAccountName is the name of the ServiceAccount to run all pods of the managed as.
	ServiceAccountName string `json:"serviceAccountName,omitempty"`

	// Volumes is a list of volumes that can be mounted by containers in a managed.
	Volumes []apiv1.Volume `json:"volumes,omitempty"`

	// VolumeClaimTemplates is a list of claims that containers are allowed to reference.
	// The Managed controller will create the claims at the beginning of the managed
	// and delete the claims upon completion of the managed
	VolumeClaimTemplates []apiv1.PersistentVolumeClaim `json:"volumeClaimTemplates,omitempty"`

	// Parallelism limits the max total parallel pods that can execute at the same time in a managed
	Parallelism *int64 `json:"parallelism,omitempty"`

	// Suspend will suspend the managed and prevent execution of any future steps in the managed
	Suspend *bool `json:"suspend,omitempty"`

	// NodeSelector is a selector which will result in all pods of the managed
	// to be scheduled on the selected node(s). This is able to be overridden by
	// a nodeSelector specified in the template.
	NodeSelector map[string]string `json:"nodeSelector,omitempty"`

	// Affinity sets the scheduling constraints for all pods in the managed.
	// Can be overridden by an affinity specified in the template
	Affinity *apiv1.Affinity `json:"affinity,omitempty"`

	// Tolerations to apply to managed pods.
	...
	// managed, irrespective of the success, failure, or error of the
	// primary managed.
	OnExit string `json:"onExit,omitempty"`
}

// Template is a reusable and composable unit of execution in a managed
type Template struct {
	...
	// NodeSelector is a selector to schedule this step of the managed to be
	// run on the selected node(s). Overrides the selector set at the managed level.
	NodeSelector map[string]string `json:"nodeSelector,omitempty"`

	// Affinity sets the pod's scheduling constraints
	// Overrides the affinity set at the managed level (if any)
	Affinity *apiv1.Affinity `json:"affinity,omitempty"`

	// Metdata sets the pods's metadata, i.e. annotations and labels
	Metadata Metadata `json:"metadata,omitempty"`

	// Deamon will allow a managed to proceed to the next step so long as the container reaches readiness
	Daemon *bool `json:"daemon,omitempty"`

	// Steps define a series of sequential/parallel managed steps
	Steps [][]ManagedStep `json:"steps,omitempty"`
	...
	// Suspend template subtype which can suspend a managed when reaching the step
	...
	// <managedname>/<nodename> in the key.
...
type Parameter struct {
	...
	// '{{managed.outputs.parameters.XXXX}} and in managed.status.outputs.parameters
...
type Artifact struct {
	...
	// '{{managed.outputs.artifacts.XXXX}} and in managed.status.outputs.artifacts
	GlobalName string `json:"globalName,omitempty"`
}
...
// of a single managed step, which the executor will use as a default location to store its files.
...
// ManagedStep is a reference to a template to execute in a series of step
...
// Item expands a single managed step into multiple parallel steps
...
type Arguments struct {
	// Parameters is the list of parameters to pass to the template or managed
	Parameters []Parameter `json:"parameters,omitempty"`

	// Artifacts is the list of artifacts to pass to the template or managed
	Artifacts []Artifact `json:"artifacts,omitempty"`
}
...
// ManagedStatus contains overall status information about a managed
// +k8s:openapi-gen=false
type ManagedStatus struct {
	// Phase a simple, high-level summary of where the managed is in its lifecycle.
	Phase NodePhase `json:"phase,omitempty"`

	// Time at which this managed started
	StartedAt metav1.Time `json:"startedAt,omitempty"`

	// Time at which this managed completed
	FinishedAt metav1.Time `json:"finishedAt,omitempty"`

	// A human readable message indicating details about why the managed is in this condition.
	Message string `json:"message,omitempty"`

	// Nodes is a mapping between a node ID and the node's status.
	Nodes map[string]NodeStatus `json:"nodes,omitempty"`

	// PersistentVolumeClaims tracks all PVCs that were created as part of the managed.
	// The contents of this list are drained at the end of the managed.
	PersistentVolumeClaims []apiv1.Volume `json:"persistentVolumeClaims,omitempty"`

	// Outputs captures output values and artifact locations produced by the managed via global outputs
	Outputs *Outputs `json:"outputs,omitempty"`
}

// RetryStrategy provides controls on how to retry a managed step
type RetryStrategy struct {
	// Limit is the maximum number of attempts when retrying a container
	Limit *int32 `json:"limit,omitempty"`
}

// NodeStatus contains status information about an individual node in the managed
// +k8s:openapi-gen=false
...
// SuspendTemplate is a template subtype to suspend a managed at a predetermined point in time
...
func (wf *Managed) GetTemplate(name string) *Template {
...
func (wf *Managed) NodeID(name string) string {
