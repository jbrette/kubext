...
// wfValidationCtx is the context for validating a managed spec
...
// ValidateManaged accepts a managed and performs validation against it. If lint is specified as
// true, will skip some validations which is permissible during linting but not submission
func ValidateManaged(wf *wfv1.Managed, lint ...bool) error {
	ctx := wfValidationCtx{
		wf:           wf,
	...
	err := validateManagedFieldNames(wf.Spec.Templates)
	if err != nil {
	...
	if linting {
		...
		// explicit value. manageds without a default value is a desired use case
	...
	ctx.globalParams[GlobalVarManagedName] = placeholderValue
	ctx.globalParams[GlobalVarManagedNamespace] = placeholderValue
	ctx.globalParams[GlobalVarManagedUID] = placeholderValue
	for _, param := range ctx.wf.Spec.Arguments.Parameters {
		ctx.globalParams["managed.parameters."+param.Name] = placeholderValue
	}
	...
	if ctx.wf.Spec.OnExit != "" {
		...
		// now when validating onExit, {{managed.status}} is now available as a global
		ctx.globalParams[GlobalVarManagedStatus] = placeholderValue
...
func validateInputs(tmpl *wfv1.Template) (map[string]interface{}, error) {
	err := validateManagedFieldNames(tmpl.Inputs.Parameters)
	if err != nil {
		return nil, errors.Errorf(errors.CodeBadRequest, "templates.%s.inputs.parameters%s", tmpl.Name, err.Error())
	}
	err = validateManagedFieldNames(tmpl.Inputs.Artifacts)
	if err != nil {
...
func validateArgumentsFieldNames(prefix string, arguments wfv1.Arguments) error {
	...
	for fieldName, lst := range fieldToSlices {
		err := validateManagedFieldNames(lst)
		if err != nil {
...
func (ctx *wfValidationCtx) validateSteps(scope map[string]interface{}, tmpl *wfv1.Template) error {
	...
	for i, stepGroup := range tmpl.Steps {
		for _, step := range stepGroup {
			...
			if errs := IsValidManagedFieldName(step.Name); len(errs) != 0 {
...
func addItemsToScope(step *wfv1.ManagedStep, scope map[string]interface{}) error {
...
func (ctx *wfValidationCtx) addOutputsToScope(templateName string, prefix string, scope map[string]interface{}) {
	...
	for _, param := range tmpl.Outputs.Parameters {
		...
		if param.GlobalName != "" && !isParameter(param.GlobalName) {
			globalParamName := fmt.Sprintf("managed.outputs.parameters.%s", param.GlobalName)
	...
	for _, art := range tmpl.Outputs.Artifacts {
		...
		if art.GlobalName != "" && !isParameter(art.GlobalName) {
			scope[fmt.Sprintf("managed.outputs.artifacts.%s", art.GlobalName)] = true
		}
	}
}

func validateOutputs(scope map[string]interface{}, tmpl *wfv1.Template) error {
	err := validateManagedFieldNames(tmpl.Outputs.Parameters)
	if err != nil {
		return errors.Errorf(errors.CodeBadRequest, "templates.%s.outputs.parameters%s", tmpl.Name, err.Error())
	}
	err = validateManagedFieldNames(tmpl.Outputs.Artifacts)
	if err != nil {
	...
	for _, art := range tmpl.Outputs.Artifacts {
		...
		if art.GlobalName != "" && !isParameter(art.GlobalName) {
			errs := IsValidManagedFieldName(art.GlobalName)
			if len(errs) > 0 {
	...
	for _, param := range tmpl.Outputs.Parameters {
		...
		if param.GlobalName != "" && !isParameter(param.GlobalName) {
			errs := IsValidManagedFieldName(param.GlobalName)
			if len(errs) > 0 {
...
// validateManagedFieldNames accepts a slice of structs and
// verifies that the Name field of the structs are:
// * unique
// * non-empty
// * matches matches our regex requirements
func validateManagedFieldNames(slice interface{}) error {
	s := reflect.ValueOf(slice)
	if s.Kind() != reflect.Slice {
		return errors.InternalErrorf("validateManagedFieldNames given a non-slice type")
	}
	...
	for i, item := range items {
		...
		if errs := IsValidManagedFieldName(name); len(errs) != 0 {
...
func (ctx *wfValidationCtx) validateDAG(scope map[string]interface{}, tmpl *wfv1.Template) error {
	...
	err = validateManagedFieldNames(tmpl.DAG.Tasks)
	if err != nil {
