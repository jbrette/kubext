...
// DefaultConfigMapName returns a formulated name for a configmap name based on the managed-controller deployment name
...
// * global parameters (e.g. {{managed.parameters.XX}}, {{managed.name}}, {{managed.status}})
...
const managedFieldNameFmt string = "[a-zA-Z0-9][-a-zA-Z0-9]*"
const managedFieldNameErrMsg string = "name must consist of alpha-numeric characters or '-', and must start with an alpha-numeric character"
const managedFieldMaxLength int = 128

var managedFieldNameRegexp = regexp.MustCompile("^" + managedFieldNameFmt + "$")

// IsValidManagedFieldName : managed field name must consist of alpha-numeric characters or '-', and must start with an alpha-numeric character
func IsValidManagedFieldName(name string) []string {
	var errs []string
	if len(name) > managedFieldMaxLength {
		errs = append(errs, apivalidation.MaxLenError(managedFieldMaxLength))
	}
	if !managedFieldNameRegexp.MatchString(name) {
		msg := managedFieldNameErrMsg + " (e.g. My-name1-2, 123-NAME)"
...
var errSuspendedCompletedManaged = errors.Errorf(errors.CodeBadRequest, "cannot suspend completed manageds")

// IsManagedSuspended returns whether or not a managed is considered suspended
func IsManagedSuspended(wf *wfv1.Managed) bool {
...
// IsManagedCompleted returns whether or not a managed is considered completed
func IsManagedCompleted(wf *wfv1.Managed) bool {
...
// SuspendManaged suspends a managed by setting spec.suspend to true. Retries conflict errors
func SuspendManaged(wfIf v1alpha1.ManagedInterface, managedName string) error {
	err := wait.ExponentialBackoff(retry.DefaultRetry, func() (bool, error) {
		wf, err := wfIf.Get(managedName, metav1.GetOptions{})
		if err != nil {
			return false, err
		}
		if IsManagedCompleted(wf) {
			return false, errSuspendedCompletedManaged
		}
...
// ResumeManaged resumes a managed by setting spec.suspend to nil and any suspended nodes to Successful.
// Retries conflict errors
func ResumeManaged(wfIf v1alpha1.ManagedInterface, managedName string) error {
	err := wait.ExponentialBackoff(retry.DefaultRetry, func() (bool, error) {
		wf, err := wfIf.Get(managedName, metav1.GetOptions{})
		if err != nil {
			return false, err
		...
		// To resume a managed with a suspended node we simply mark the node as Successful
...
// FormulateResubmitManaged formulate a new managed from a previous managed, optionally re-using successful nodes
func FormulateResubmitManaged(wf *wfv1.Managed, memoized bool) (*wfv1.Managed, error) {
	newWF := wfv1.Managed{}
	newWF.TypeMeta = wf.TypeMeta

	// Resubmitted managed will use generated names
	...
	// When resubmitting managed with memoized nodes, we need to use a predetermined managed name
	...
	if memoized {
		...
		default:
			return nil, errors.Errorf(errors.CodeBadRequest, "managed must be Failed/Error to resubmit in memoized mode")
		}
	...
	// carry over user labels and annotations from previous managed.
	// skip any jbrette.io labels except for the controller instanceID label.
	for key, val := range wf.ObjectMeta.Labels {
		if strings.HasPrefix(key, managed.FullName+"/") && key != LabelKeyControllerInstanceID {
			continue
		}
	...
	for _, node := range wf.Status.Nodes {
		...
		case wfv1.NodeError, wfv1.NodeFailed, wfv1.NodeRunning:
			...
			// mismanages state (e.g. panic when operating on a managed)
		default:
			return nil, errors.InternalErrorf("Managed cannot be resubmitted with nodes in %s phase", node, node.Phase)
		}
	}
	return &newWF, nil
}
...
func convertNodeID(newWf *wfv1.Managed, regex *regexp.Regexp, oldNodeID string, oldNodes map[string]wfv1.NodeStatus) string {
	node := oldNodes[oldNodeID]
	newNodeName := regex.ReplaceAllString(node.Name, newWf.ObjectMeta.Name)
	return newWf.NodeID(newNodeName)
}

// RetryManaged updates a managed, deleting all failed steps as well as the onExit node (and children)
func RetryManaged(kubeClient kubernetes.Interface, wfClient v1alpha1.ManagedInterface, wf *wfv1.Managed) (*wfv1.Managed, error) {
	switch wf.Status.Phase {
	case wfv1.NodeFailed, wfv1.NodeError:
	default:
		return nil, errors.Errorf(errors.CodeBadRequest, "managed must be Failed/Error to retry")
	}
	newWF := wf.DeepCopy()
	podIf := kubeClient.CoreV1().Pods(wf.ObjectMeta.Namespace)

	// Delete/reset fields which indicate managed completed
	...
	for _, node := range wf.Status.Nodes {
		...
		case wfv1.NodeError, wfv1.NodeFailed:
			...
			// mismanages state (e.g. panic when operating on a managed)
		default:
			return nil, errors.InternalErrorf("Managed cannot be retried with nodes in %s phase", node, node.Phase)
		}
		if node.Type == wfv1.NodeTypePod {
