...
func (woc *wfOperationCtx) executeSteps(nodeName string, tmpl *wfv1.Template, boundaryID string) *wfv1.NodeStatus {
	...
	for i, stepGroup := range tmpl.Steps {
		...
		if !sgNode.Completed() {
			woc.log.Infof("Managed step group node %v not yet completed", sgNode)
			return node
		}
...
func (woc *wfOperationCtx) executeStepGroup(stepGroup []wfv1.ManagedStep, sgNodeName string, stepsCtx *stepsContext) *wfv1.NodeStatus {
...
func (woc *wfOperationCtx) resolveReferences(stepGroup []wfv1.ManagedStep, scope *wfScope) ([]wfv1.ManagedStep, error) {
	newStepGroup := make([]wfv1.ManagedStep, len(stepGroup))
...
func (woc *wfOperationCtx) expandStepGroup(stepGroup []wfv1.ManagedStep) ([]wfv1.ManagedStep, error) {
	newStepGroup := make([]wfv1.ManagedStep, 0)
	for _, step := range stepGroup {
...
func (woc *wfOperationCtx) expandStep(step wfv1.ManagedStep) ([]wfv1.ManagedStep, error) {
	stepBytes, err := json.Marshal(step)
	if err != nil {
		return nil, errors.InternalWrapError(err)
	}
	fstTmpl := fasttemplate.New(string(stepBytes), "{{", "}}")
	expandedStep := make([]wfv1.ManagedStep, 0)
	var items []wfv1.Item
