...
// Reusable k8s pod spec portions used in managed pods
var (
...
func (woc *wfOperationCtx) createManagedPod(nodeName string, mainCtr apiv1.Container, tmpl *wfv1.Template) (*apiv1.Pod, error) {
	...
	pod := apiv1.Pod{
		ObjectMeta: metav1.ObjectMeta{
			...
			Labels: map[string]string{
				common.LabelKeyManaged:  woc.wf.ObjectMeta.Name, // Allows filtering by pods related to specific managed
				common.LabelKeyCompleted: "false",                // Allows filtering by incomplete managed pods
			},
	...
	if tmpl.GetType() != wfv1.TemplateTypeResource {
		...
		// kubextexec runs as the main container and will perform the job of
	...
	if err != nil {
		if apierr.IsAlreadyExists(err) {
			// managed pod names are deterministic. We can get here if the
			// controller fails to persist the managed after creating the pod.
...
// substituteGlobals returns a managed spec with global parameter references substituted
func substituteGlobals(wfSpec *wfv1.ManagedSpec, globalParams map[string]string) (*wfv1.ManagedSpec, error) {
	...
	var newWfSpec wfv1.ManagedSpec
...
// addSchedulingConstraints applies any node selectors or affinity rules to the pod, either set in the managed or the template
func addSchedulingConstraints(pod *apiv1.Pod, wfSpec *wfv1.ManagedSpec, tmpl *wfv1.Template) {
...
// These are either specified in the managed.spec.volumes or the managed.spec.volumeClaimTemplate section
func addVolumeReferences(pod *apiv1.Pod, wfSpec *wfv1.ManagedSpec, tmpl *wfv1.Template, pvcs []apiv1.Volume) error {
	switch tmpl.GetType() {
	...
	addVolumeRef := func(volMounts []apiv1.VolumeMount) error {
		for _, volMnt := range volMounts {
			...
			if vol == nil {
				return errors.Errorf(errors.CodeBadRequest, "volume '%s' not found in managed spec", volMnt.Name)
			}
			found := false
...
func (woc *wfOperationCtx) addArchiveLocation(pod *apiv1.Pod, tmpl *wfv1.Template) error {
	...
	// (e.g. mymanagedartifacts/kubext-wf-fhljp/kubext-wf-fhljp-123291312382/src.tgz)
	// TODO: will need to support more advanced organization of artifacts such as dated
	// (e.g. mymanagedartifacts/2017/10/31/... )
