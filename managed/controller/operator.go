...
import (
	...
	"github.com/jbrette/kubext/util/retry"
...
// wfOperationCtx is the context for evaluation and operation of a single managed
type wfOperationCtx struct {
	// wf is the managed object
	wf *wfv1.Managed
	// orig is the original managed object for purposes of creating a patch
	orig *wfv1.Managed
	// updated indicates whether or not the managed object itself was updated
	// and needs to be persisted back to kubernetes
	updated bool
	// log is an logrus logging context to corralate logs with a managed
	log *log.Entry
	// controller reference to managed controller
	controller *ManagedController
	// globalParams holds any parameters that are available to be referenced
	// in the global scope (e.g. managed.parameters.XXX).
	globalParams map[string]string
	// map of pods which need to be labeled with completed=true
	completedPods map[string]bool
	// deadline is the dealine time in which this operation should relinquish
	// its hold on the managed so that an operation does not run for too long
	// and starve other workqueue items. It also enables managed progress to
...
var (
	...
	// ErrParallelismReached indicates this managed reached its parallelism limit
	ErrParallelismReached = errors.New(errors.CodeForbidden, "Max parallelism reached")
)

// maxOperationTime is the maximum time a managed operation is allowed to run
// for before requeuing the managed onto the workqueue.
const maxOperationTime time.Duration = 10 * time.Second

// wfScope contains the current scope of variables available when iterating steps in a managed
type wfScope struct {
	tmpl  *wfv1.Template
	scope map[string]interface{}
}

// newManagedOperationCtx creates and initializes a new wfOperationCtx object.
func newManagedOperationCtx(wf *wfv1.Managed, wfc *ManagedController) *wfOperationCtx {
	// NEVER modify objects from the store. It's a read-only, local cache.
	// You can use DeepCopy() to make a deep copy of original object and modify this copy
	// Or create a copy manually for better performance
	woc := wfOperationCtx{
		wf:      wf.DeepCopyObject().(*wfv1.Managed),
		orig:    wf,
		updated: false,
		log: log.WithFields(log.Fields{
			"managed":  wf.ObjectMeta.Name,
...
// operate is the main operator logic of a managed. It evaluates the current state of the managed,
// and its pods and decides how to proceed down the execution path.
// TODO: an error returned by this method should result in requeuing the managed to be retried at a
// later time
func (woc *wfOperationCtx) operate() {
	...
	defer func() {
		if r := recover(); r != nil {
			if rerr, ok := r.(error); ok {
				woc.markManagedError(rerr, true)
			} else {
				woc.markManagedPhase(wfv1.NodeError, true, fmt.Sprintf("%v", r))
			}
			woc.log.Errorf("Recovered from panic: %+v\n%s", r, debug.Stack())
		}
	}()
	woc.log.Infof("Processing managed")
	// Perform one-time managed validation
	if woc.wf.Status.Phase == "" {
		woc.markManagedRunning()
		err := common.ValidateManaged(woc.wf)
		if err != nil {
	...
	if woc.wf.Spec.Suspend != nil && *woc.wf.Spec.Suspend {
		woc.log.Infof("managed suspended")
		return
	}
	...
	if err != nil {
		...
		woc.markManagedError(err, true)
		return
	}
	var managedStatus wfv1.NodePhase
	var managedMessage string
	node, _ := woc.executeTemplate(woc.wf.Spec.Entrypoint, woc.wf.Spec.Arguments, woc.wf.ObjectMeta.Name, "")
	if node == nil || !node.Completed() {
		// node can be nil if a managed created immediately in a parallelism == 0 state
		return
	}
	managedStatus = node.Phase
	managedMessage = node.Message

	var onExitNode *wfv1.NodeStatus
	if woc.wf.Spec.OnExit != "" {
		if managedStatus == wfv1.NodeSkipped {
			// treat skipped the same as Succeeded for managed.status
			woc.globalParams[common.GlobalVarManagedStatus] = string(wfv1.NodeSucceeded)
		} else {
			woc.globalParams[common.GlobalVarManagedStatus] = string(managedStatus)
		}
	...
	if err != nil {
		woc.log.Errorf("%s error: %+v", woc.wf.ObjectMeta.Name, err)
		// Mark the managed with an error message and return, but intentionally do not
		...
		woc.markManagedError(err, false)
		return
	}

	// If we get here, the managed completed, all PVCs were deleted successfully, and
	// exit handlers were executed. We now need to infer the managed phase from the
	// node phase.
	switch managedStatus {
	case wfv1.NodeSucceeded, wfv1.NodeSkipped:
		if onExitNode != nil && !onExitNode.Successful() {
			// if main managed succeeded, but the exit node was unsuccessful
			// the managed is now considered unsuccessful.
			woc.markManagedPhase(onExitNode.Phase, true, onExitNode.Message)
		} else {
			woc.markManagedSuccess()
		}
	case wfv1.NodeFailed:
		woc.markManagedFailed(managedMessage)
	case wfv1.NodeError:
		woc.markManagedPhase(wfv1.NodeError, true, managedMessage)
	default:
...
func (woc *wfOperationCtx) setGlobalParameters() {
	woc.globalParams[common.GlobalVarManagedName] = woc.wf.ObjectMeta.Name
	woc.globalParams[common.GlobalVarManagedNamespace] = woc.wf.ObjectMeta.Namespace
	woc.globalParams[common.GlobalVarManagedUID] = string(woc.wf.ObjectMeta.UID)
	for _, param := range woc.wf.Spec.Arguments.Parameters {
		woc.globalParams["managed.parameters."+param.Name] = *param.Value
	}
	if woc.wf.Status.Outputs != nil {
		for _, param := range woc.wf.Status.Outputs.Parameters {
			woc.globalParams["managed.outputs.parameters."+param.Name] = *param.Value
		}
	}
}
...
// persistUpdates will update a managed with any updates made during managed operation.
...
func (woc *wfOperationCtx) persistUpdates() {
	...
	woc.log.Info("Managed update successful")

	// HACK(jessesuen) after we successfully persist an update to the managed, the informer's
	// cache is now invalid. It's very common that we will need to immediately re-operate on a
	// managed due to queuing by the pod workers. The following sleep gives a *chance* for the
	// informer's cache to catch up to the version of the managed we just persisted. Without
	// this sleep, the next worker to work on this managed will very likely operate on a stale
	// object and redo work.
	time.Sleep(1 * time.Second)

	// It is important that we *never* label pods as completed until we successfully updated the managed
...
// reapplyUpdate GETs the latest version of the managed, re-applies the updates and
// retries the UPDATE multiple times. For reasoning behind this technique, see:
// https://github.com/kubernetes/community/blob/master/contributors/devel/api-conventions.md#concurrency-control-and-consistency
func (woc *wfOperationCtx) reapplyUpdate(wfClient v1alpha1.ManagedInterface) error {
	...
	// Next get latest version of the managed, apply the patch and retyr the Update
	attempt := 1
	for {
...
// requeue this managed onto the workqueue for later processing
func (woc *wfOperationCtx) requeue() {
	key, err := cache.MetaNamespaceKeyFunc(woc.wf)
	if err != nil {
		woc.log.Errorf("Failed to requeue managed %s: %v", woc.wf.ObjectMeta.Name, err)
		return
	}
...
// podReconciliation is the process by which a managed will examine all its related
// pods and update the node state before continuing the evaluation of the managed.
// Records all pods which were observed completed, which will be labeled completed=true
// after successful persist of the managed.
func (woc *wfOperationCtx) podReconciliation() error {
	podList, err := woc.getRunningManagedPods()
	if err != nil {
		return err
	}
	...
	// If we get here, our initial query for pods related to this managed returned nothing.
	...
	//  1. managed's pods are still pending (best case scenario)
	//  2. managed's pods were deleted unbeknownst to the controller
	//  3. managed's pods were marked completed=true, but we are operating on a stale managed object
	...
	// including ALL managed pods in the query. If any one of our nodes does not show up in this
	...
	podList, err = woc.getAllManagedPods()
	if err != nil {
		return err
	}
	...
	// Now iterate the managed pod nodes which we still believe to be incomplete.
...
// getRunningManagedPods returns running pods of the current managed.
func (woc *wfOperationCtx) getRunningManagedPods() (*apiv1.PodList, error) {
	options := metav1.ListOptions{
		LabelSelector: fmt.Sprintf("%s=%s,%s=false",
			common.LabelKeyManaged,
...
// getAllManagedPods returns all pods related to the current managed
func (woc *wfOperationCtx) getAllManagedPods() (*apiv1.PodList, error) {
	options := metav1.ListOptions{
...
func (woc *wfOperationCtx) createPVCs() error {
	if woc.wf.Status.Phase != wfv1.NodeRunning {
		// Only attempt to create PVCs if managed transitioned to Running state
	...
	if len(woc.wf.Spec.VolumeClaimTemplates) == len(woc.wf.Status.PersistentVolumeClaims) {
		...
		// This will also handle the case where managed has no volumeClaimTemplates.
		return nil
	}
	...
	for i, pvcTmpl := range woc.wf.Spec.VolumeClaimTemplates {
		...
		// PVC name will be <managedname>-<volumeclaimtemplatename>
...
func (woc *wfOperationCtx) executeTemplate(templateName string, args wfv1.Arguments, nodeName string, boundaryID string) (*wfv1.NodeStatus, error) {
	...
	// Check if we took too long operating on this managed and immediately return if we did
	...
	// Check if we exceeded template or managed parallelism and immediately return if we did
...
// markManagedPhase is a convenience method to set the phase of the managed with optional message
// optionally marks the managed completed, which sets the finishedAt timestamp and completed label
func (woc *wfOperationCtx) markManagedPhase(phase wfv1.NodePhase, markCompleted bool, message ...string) {
	...
	case wfv1.NodeSucceeded, wfv1.NodeFailed, wfv1.NodeError:
		if markCompleted {
			woc.log.Infof("Marking managed completed")
...
func (woc *wfOperationCtx) markManagedRunning() {
	woc.markManagedPhase(wfv1.NodeRunning, false)
}

func (woc *wfOperationCtx) markManagedSuccess() {
	woc.markManagedPhase(wfv1.NodeSucceeded, true)
}

func (woc *wfOperationCtx) markManagedFailed(message string) {
	woc.markManagedPhase(wfv1.NodeFailed, true, message)
}

func (woc *wfOperationCtx) markManagedError(err error, markCompleted bool) {
	woc.markManagedPhase(wfv1.NodeError, markCompleted, err.Error())
}
...
// checkParallelism checks if the given template is able to be executed, considering the current active pods and managed/template parallelism
func (woc *wfOperationCtx) checkParallelism(tmpl *wfv1.Template, node *wfv1.NodeStatus, boundaryID string) error {
	if woc.wf.Spec.Parallelism != nil && woc.activePods >= *woc.wf.Spec.Parallelism {
		woc.log.Infof("managed active pod spec parallelism reached %d/%d", woc.activePods, *woc.wf.Spec.Parallelism)
...
func (woc *wfOperationCtx) executeContainer(nodeName string, tmpl *wfv1.Template, boundaryID string) *wfv1.NodeStatus {
	...
	_, err := woc.createManagedPod(nodeName, *tmpl.Container, tmpl)
	if err != nil {
...
func (woc *wfOperationCtx) executeScript(nodeName string, tmpl *wfv1.Template, boundaryID string) *wfv1.NodeStatus {
	...
	_, err := woc.createManagedPod(nodeName, mainCtr, tmpl)
	if err != nil {
		return woc.initializeNode(nodeName, wfv1.NodeTypePod, tmpl.Name, boundaryID, wfv1.NodeError, err.Error())
	}
	return woc.initializeNode(nodeName, wfv1.NodeTypePod, tmpl.Name, boundaryID, wfv1.NodeRunning)
...
func (woc *wfOperationCtx) addParamToGlobalScope(param wfv1.Parameter) {
	...
	woc.updated = true
	paramName := fmt.Sprintf("managed.outputs.parameters.%s", param.GlobalName)
...
func (woc *wfOperationCtx) addArtifactToGlobalScope(art wfv1.Artifact) {
	...
	woc.updated = true
	globalArtName := fmt.Sprintf("managed.outputs.artifacts.%s", art.GlobalName)
	if woc.wf.Status.Outputs != nil {
