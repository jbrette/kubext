...
func newWoc(wfs ...wfv1.Managed) *wfOperationCtx {
	var wf *wfv1.Managed
	if len(wfs) == 0 {
	...
	woc := newManagedOperationCtx(wf, fakeController)
	return woc
}

// getPodName returns the podname of the created pod of a managed
// Only applies to single pod manageds
func getPodName(wf *wfv1.Managed) string {
	if len(wf.Status.Nodes) != 1 {
		panic("getPodName called against a multi-pod managed")
	}
...
// for the pod from managed.spec.serviceAccountName.
...
// TestImagePullSecrets verifies the ability to carry forward imagePullSecrets from managed.spec
