...
import (
	...
	artifact "github.com/jbrette/kubext/managed/artifacts"
	"github.com/jbrette/kubext/managed/artifacts/artifactory"
	"github.com/jbrette/kubext/managed/artifacts/git"
	"github.com/jbrette/kubext/managed/artifacts/http"
	"github.com/jbrette/kubext/managed/artifacts/raw"
	"github.com/jbrette/kubext/managed/artifacts/s3"
...
// ManagedExecutor is program which runs as the init/wait container
...
// NewExecutor instantiates a new managed executor
func NewExecutor(clientset kubernetes.Interface, podName, namespace, podAnnotationsPath string, cre ContainerRuntimeExecutor) ManagedExecutor {
...
func (we *ManagedExecutor) HandleError() {
	if r := recover(); r != nil {
...
func (we *ManagedExecutor) LoadArtifacts() error {
...
func (we *ManagedExecutor) StageFiles() error {
	var filePath string
	var body []byte
...
func (we *ManagedExecutor) SaveArtifacts() error {
	...
	tempOutArtDir := "/kubext/outputs/artifacts"
...
func (we *ManagedExecutor) SaveParameters() error {
...
func (we *ManagedExecutor) InitDriver(art wfv1.Artifact) (artifact.ArtifactDriver, error) {
	if art.S3 != nil {
...
func (we *ManagedExecutor) getPod() (*apiv1.Pod, error) {
...
func (we *ManagedExecutor) getSecrets(namespace, name, key string) (string, error) {
...
func (we *ManagedExecutor) GetMainContainerStatus() (*apiv1.ContainerStatus, error) {
	pod, err := we.getPod()
	if err != nil {
		return nil, err
	}
...
func (we *ManagedExecutor) GetMainContainerID() (string, error) {
...
func (we *ManagedExecutor) CaptureScriptResult() error {
...
func (we *ManagedExecutor) AnnotateOutputs() error {
...
func (we *ManagedExecutor) AddError(err error) {
	we.errors = append(we.errors, err)
}

// AddAnnotation adds an annotation to the managed pod
func (we *ManagedExecutor) AddAnnotation(key, value string) error {
...
func (we *ManagedExecutor) Wait() (err error) {
	defer func() {
...
func (we *ManagedExecutor) waitMainContainerStart() (string, error) {
	for {
		ctrStatus, err := we.GetMainContainerStatus()
		if err != nil {
			return "", err
...
func (we *ManagedExecutor) monitorAnnotations(ctx context.Context) <-chan struct{} {
...
func (we *ManagedExecutor) monitorDeadline(ctx context.Context, annotationsUpdate <-chan struct{}) {
			...
			if we.ExecutionControl != nil && we.ExecutionControl.Deadline != nil {
				if time.Now().UTC().After(*we.ExecutionControl.Deadline) {
					if !we.ExecutionControl.Deadline.IsZero() {
						...
						log.Warnf(errMsg)
						// TODO(jessesuen): we do not have managed or step level timeouts (yet) so do not annotate yet
...
func (we *ManagedExecutor) killSidecars() error {
...
func (we *ManagedExecutor) LoadTemplate() error {
...
func (we *ManagedExecutor) LoadExecutionControl() error {
